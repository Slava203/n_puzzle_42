	...
	pub fn is_match(&self, to_match: &String) -> Option<Token<E>>
	{
		if self.re.is_match(to_match){
			...
			if pos.is_some(){
				...
				if begin == 0{
					let content = fn_str::sub_string(to_match, begin, end)
							.unwrap();
...
impl<E: Clone> Tokenizer<E>
{
	...
	pub fn set_discards(&mut self, reg: Regex) {
		self.discards = reg;
	}
	...
	{
		let reduced = fn_str::sub_string(to_split, token_len, to_split.len() - token_len)
				.unwrap();
